...
# Log images
def log_input_image(x, label_nc=0):
    if label_nc == 0:
        return tensor2im(x)
    elif label_nc == 1:
...
def vis_faces_v2(log_hooks):
    ...
    for i in range(display_count):
        ...
        if 'diff_input' in hooks_dict:
            ...
            vis_faces_no_id_v2(hooks_dict, fig, gs, i)
    plt.tight_layout()
    return fig


def vis_faces_v3(log_hooks):
    display_count = len(log_hooks)
    fig = plt.figure(figsize=(12, 4 * display_count))
    ...
    for i in range(display_count):
        ...
        if 'diff_input' in hooks_dict:
            ...
            vis_faces_no_id_v3(hooks_dict, fig, gs, i)
    plt.tight_layout()
    return fig


def vis_faces_v4(log_hooks):
    display_count = len(log_hooks)
    ...
    for i in range(display_count):
        ...
        if 'diff_input' in hooks_dict:
            ...
            vis_faces_no_id_v4(hooks_dict, fig, gs, i)
    plt.tight_layout()
    return fig
...
def vis_faces_no_id_v2(hooks_dict, fig, gs, i):
    plt.imshow(hooks_dict['source_face'], cmap="gray")
    plt.title('source_face')
    fig.add_subplot(gs[i, 1])
    plt.imshow(hooks_dict['drv1_face'])
    plt.title('drv1_face')
    fig.add_subplot(gs[i, 2])
    plt.imshow(hooks_dict['drv2_face'])
    plt.title('drv2_face')
...
def vis_faces_no_id_v3(hooks_dict, fig, gs, i):
    plt.imshow(hooks_dict['input_face'], cmap="gray")
    plt.title('input_face')

    fig.add_subplot(gs[i, 1])
    plt.imshow(hooks_dict['target_face_scale'])
    plt.title('target_face_scale')

    fig.add_subplot(gs[i, 2])
    plt.imshow(hooks_dict['target_face_gt'])
    plt.title('target_face_gt')

    fig.add_subplot(gs[i, 3])
    plt.imshow(hooks_dict['output_face'])
    plt.title('output_face')


def vis_faces_no_id_v4(hooks_dict, fig, gs, i):
    plt.imshow(hooks_dict['source_face'], cmap="gray")
    plt.title('source_face')

    fig.add_subplot(gs[i, 1])
    plt.imshow(hooks_dict['drv1_face_input'])
    plt.title('drv1_face_input')

    fig.add_subplot(gs[i, 2])
    plt.imshow(hooks_dict['drv1_face_gt'])
    plt.title('drv1_face_gt')
