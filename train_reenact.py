...
from training.model import Discriminator, Encoder_32, Generator_32_att
from training.pose import Encoder_Pose
from training.dataset import Dataset_scale_trans
from utils import common

torch.backends.cudnn.benchmark = True


def log_images_v3(step, logdir, name, im_data, subscript=None, log_latest=False):
    fig = common.vis_faces_v3(im_data)
    if log_latest:
        step = 0
    if subscript:
...
def parse_and_log_images_v3(step, logdir, x, y_scale, y_gt, y_hat, title, subscript=None, display_count=2):
    im_data = []
    for i in range(display_count):
        cur_im_data = {
            'input_face': common.log_input_image(x[i]),
            'target_face_scale': common.tensor2im(y_scale[i]),
            'target_face_gt': common.tensor2im(y_gt[i]),
            'output_face': common.tensor2im(y_hat[i]),
        }
        im_data.append(cur_im_data)
    log_images_v3(step, logdir, title, im_data=im_data, subscript=subscript)
...
def setup(rank, world_size):
    ...
    os.environ["MASTER_PORT"] = "12345"  # 12369
...
class DDPModel(nn.Module):
    def __init__(self, device, args):
        ...
        self.generator = Generator_32_att(
            args.size,
    ...
    def forward(self, real_img, mode):
        if mode == "D":
            ...
            with torch.no_grad():
                src = real_img[0]
                drv_gt = real_img[1]
                drv_input = real_img[2]

                src_w = self.encoder(src)
                flow, pose = self.encoder_p(drv_input)
                fake_img = self.generator([src_w, flow])

            real_pred = self.discriminator(src)
            ...
            x_rec_loss = self.mse_loss(drv_gt, fake_img)
            perceptual_loss = self.percept(drv_gt, fake_img).mean()

            fake_pred_from_E = self.discriminator(fake_img)

            indomainGAN_E_loss = F.softplus(-fake_pred_from_E).mean()

            return x_rec_loss, perceptual_loss, indomainGAN_E_loss, fake_img
            ...
            flow, pose = self.e_ema_p(drv_input)
            fake_img = self.g_ema([src_w, flow])

            x_rec_loss = self.mse_loss(drv_gt, fake_img)
            perceptual_loss = self.percept(drv_gt, fake_img).mean()

            return x_rec_loss, perceptual_loss, fake_img
...
def ddp_main(rank, world_size, args):
    ...
    if args.ckpt:
        ...
        model.module.encoder_p.load_state_dict(ckpt["encoder_p"])
        model.module.e_ema_p.load_state_dict(ckpt["e_ema_p"])
    ...
    train_dataset = Dataset_scale_trans(args.train_path, train=True, transforms=transform)
    val_dataset = Dataset_scale_trans(args.val_path, train=False, transforms=transform)
    ...
    # requires_grad(model.module.vgg, False)
    epoch = -1
    ...
    for i in pbar:
        ...
        [src_img, drv_img_gt, drv_img_input], _ = next(train_loader)
        src_img = src_img.to(map_location)
        drv_img_gt = drv_img_gt.to(map_location)
        drv_img_input = drv_img_input.to(map_location)

        requires_grad(model.module.discriminator, True)
        # D adv
        d_loss = model([src_img, drv_img_gt, drv_img_input], "D")
        d_loss = d_loss.mean()
        ...
        if d_regularize:
            d_reg_loss, r1_loss = model(src_img, "D_reg")  # r1 loss
        ...
        # E_x_rec,
        x_rec_loss, perceptual_loss, indomainGAN_E_loss, fake_img = model([src_img, drv_img_gt, drv_img_input], "E_x_rec")
        ...
        pbar.set_description(
            (
                f"g: {indomainGAN_E_loss.item():.4f}; d: {d_loss.item():.4f}; r1: {r1_val:.4f}; rec: {x_rec_loss.item():.4f}; per: {perceptual_loss.item():.4f}")
        )

        # log image
        if rank == 0:
            if i % args.image_interval == 0 or (i < 1000 and i % 25 == 0):
                parse_and_log_images_v3(i, save_dir, src_img, drv_img_input, drv_img_gt, fake_img, title='imgs/train/')
        ...
        with torch.no_grad():
            ...
            if i % args.save_img_interval == 0:
                ...
                iter_num = 0
                for ([src_img, drv_img_gt, drv_img_input], [s1, d1]) in tqdm(val_loader):

                    src_img = src_img.to(map_location)
                    drv_img_gt = drv_img_gt.to(map_location)
                    drv_img_input = drv_img_input.to(map_location)

                    x_rec_loss, perceptual_loss, fake_img = model([src_img, drv_img_gt, drv_img_input], "cal_mse_lpips")
                    ...
                    # log images
                    if rank == 0:
                        parse_and_log_images_v3(i, save_dir, src_img, drv_img_input, drv_img_gt, fake_img,
                                                title='imgs/test/')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkname", type=str, default='exp-reenact-rec5-p5')
    parser.add_argument("--describ", type=str, default='aug fix')
    parser.add_argument("--train_path", type=str, default='data/voxceleb/vox2-png/')
    parser.add_argument("--val_path", type=str, default='data/voxceleb/vox2-png/')
    ...
    parser.add_argument("--lambda_x_rec_loss", type=float, default=5)
    parser.add_argument("--lambda_d_loss", type=float, default=1)
    parser.add_argument("--lambda_perceptual_loss", type=float, default=5)
